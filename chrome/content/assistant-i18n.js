var ZoteroAssistantI18n = (() => {
  ...
  const TEXT_EN = {
    ...
    "拒绝": "Reject",
    "允许写入该导出文件": "Allow Writing This Export File",
    "允许执行该批量计划": "Allow Executing This Batch Plan",
    "列出可用导出格式": "List Available Export Formats",
    "批量计划": "Batch Plan",
    "查看详情": "View Details",
    ...
    "联网元数据源（lookup_metadata_candidates）": "Online metadata sources (lookup_metadata_candidates)",
    "默认启用 Crossref、DataCite、OpenAlex、arXiv、Open Library、Google Books。下面的专业源默认关闭；启用后会在联网补元数据时一起查询。": "Crossref, DataCite, OpenAlex, arXiv, Open Library, and Google Books are enabled by default. The specialist sources below are disabled by default and are queried only when enabled.",
    "启用 Semantic Scholar": "Enable Semantic Scholar",
    "Semantic Scholar API Key（可选）": "Semantic Scholar API Key (optional)",
    "启用 PubMed / NCBI": "Enable PubMed / NCBI",
    "PubMed API Key（可选）": "PubMed API Key (optional)",
    "PubMed Email（可选，NCBI 建议提供）": "PubMed Email (optional; recommended by NCBI)",
    "工具只返回候选元数据，不直接写入 Zotero。若需要读取 PDF/EPUB 开头片段来生成查询线索，在确认或审核模式下会先请求授权。": "The tool only returns metadata candidates and never writes to Zotero. If it needs to read a leading PDF/EPUB snippet to build query clues, it asks for approval in confirm or review mode.",
