var ZoteroAssistantPluginModel = (() => {
  ...
  systemPrompt() {
    return [
      ...
      `Bibliographic web lookup: for papers, books, DOI/arXiv/ISBN/PMID, or supplementing metadata, use lookup_metadata_candidates (read-only; never writes). With Zotero items or attachments: read_item_fields first, then pass up to ${MAX_METADATA_LOOKUP_ITEMS} itemKeys (useTextProbe defaults true). With no library item yet: pass query (min 2 chars) and optional hints (creators, year, doi, etc.) — do not use live_search for bibliographic search. live_search is only for non-bibliographic public web (sites, news, docs). Use high-confidence matches to propose update_metadata for existing items, create_parent_item for top-level attachments, create_item for a new top-level bibliographic item, or create_note for evidence notes; if confidence is low or candidates tie, ask the user to choose.`,
      ...
      `Web: use live_search only for non-bibliographic public web queries (max ${MAX_LIVE_SEARCH_PER_MODEL_ROUND} per model round). Do not use live_search to find paper/book metadata — use lookup_metadata_candidates with query instead. The old tool name web_search is invalid. Use web_fetch to read a specific public URL as markdown (max ${MAX_WEB_FETCH_PER_MODEL_ROUND} per model round). Cite sources. Do not fetch login-only or private URLs.`,
      ...
      "Creating a bibliographic item from scratch is high risk: only call create_item after the user has selected or accepted specific metadata from lookup_metadata_candidates or explicitly provided the fields. Do not use create_item to create attachments, notes, annotations, or downloaded files.",
      ...
      `Limits: per model round (each model call cycle), at most ${MAX_COLLECTIONS_PER_MODEL_ROUND} new collections, ${MAX_CREATED_ITEMS_PER_MODEL_ROUND} new bibliographic items via create_item, and ${MAX_ITEMS_PER_MODEL_ROUND} items in batch write tools. The whole task may run up to ${MAX_TASK_LOOPS} model rounds.`,
